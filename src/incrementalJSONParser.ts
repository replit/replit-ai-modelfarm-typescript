 ...
 * after every chunk
 */
export default class IncrementalJSONParser<T = any> {
  ...
  private *writeGenerator(chunk: string) {
        ...
        for (; this.bufferIndex < workData.length; this.bufferIndex++) {
          ...
          if (c === '{') {
            ...
            if (this.level === 0) {
              ...
              const parsed = JSON.parse(workData.substring(lastLevel0, this.bufferIndex + 1));
              yield parsed;
